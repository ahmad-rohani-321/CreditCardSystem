            ...
            if (Validated())
            {
                if (_id == 0)
                {
                    ...
                    bool added = _parties.AddParty(
                        new() { CreationDate = DateTime.Now, Name = txtName.Text, PartyType = txtType.Text, Remarks = txtRemarks.Text, PhoneNumbers = number }, txtFirstBalance.Value);
                    if (added)
                        Defaults.MessageBox();
                    else
